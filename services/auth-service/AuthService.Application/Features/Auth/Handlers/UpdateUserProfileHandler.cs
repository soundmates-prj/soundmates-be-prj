...
public sealed class UpdateUserProfileHandler : ICommandHandler<UpdateUserProfileCommand, UserProfileResult>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(command.FirstName) || !string.IsNullOrWhiteSpace(command.LastName))
        {
            ...
            catch (AuthService.Domain.Exceptions.UserValidationException ex)
            {
        ...
        if (command.Gender != null)
        {
            ...
            if (Enum.TryParse<AuthService.Domain.Enums.Gender>(genderValue, true, out var gender))
            {
        ...
        // Publish typed UserProfileUpdatedEvent (Auth — domain state change)
        ...
        await _outbox.EnqueueAsync(RoutingKeys.Auth.UserProfileUpdated, evt, cancellationToken);

        return Result<UserProfileResult>.Success(user.ToProfileResult(profile), "Profile updated successfully");
    }
}