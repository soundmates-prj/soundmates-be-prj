...
{
    /// <summary>
    /// Activity/Security Events Projection Service
    /// Tracks login attempts, registration failures, security events, etc.
    /// </summary>
    public sealed class RabbitMqActivityProjectionService : BackgroundService
    {
        private readonly ILogger<RabbitMqActivityProjectionService> _logger;
        ...
        private const string QueueName = "auth-service-query.activity";
        private const int MaxRetryAttempts = 30;
        private const int RetryDelaySeconds = 10;

        public RabbitMqActivityProjectionService(
            IConfiguration cfg,
            ILogger<RabbitMqActivityProjectionService> logger,
            IServiceProvider serviceProvider)
        {
            ...
            var handlers = scope.ServiceProvider.GetServices<IActivityEventHandler>();
            foreach (var handler in handlers)
            {
                _handlerTypes[handler.EventType] = handler.GetType();
            }

            _logger.LogInformation("RabbitMQ Activity Service Config: Host={Host}, Port={Port}, Handlers={Count}", 
        ...
        {
            _logger.LogInformation("🔐 Starting Activity Projection Service...");
            ...
            try
            {
                ...
                _logger.LogInformation("✅ Activity Service started successfully!");
                await Task.Delay(Timeout.Infinite, stoppingToken);
            }
            catch (OperationCanceledException)
            {
                _logger.LogInformation("🛑 Activity Service stopping...");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "💥 Fatal error in Activity Service");
                throw;
            }
        }
        ...
        private async Task ConnectWithRetryAsync(CancellationToken ct)
        {
            for (int attempt = 1; attempt <= MaxRetryAttempts; attempt++)
            {
                ...
                try
                {
                    _conn = await _factory.CreateConnectionAsync("auth-query-activity", cancellationToken: ct);
                    _channel = await _conn.CreateChannelAsync(cancellationToken: ct);
                    _logger.LogInformation("✅ Activity Service connected on attempt {Attempt}", attempt);
                    return;
                }
                catch (Exception ex) when (ex is BrokerUnreachableException or SocketException or TimeoutException)
                {
                    if (attempt % 5 == 0)
                        _logger.LogWarning("⏳ Activity Service attempt {Attempt}/{Max}: {Message}", 
            ...
            _logger.LogError("❌ Activity Service failed after {Attempts} attempts", MaxRetryAttempts);
        }
        ...
        private async Task SetupQueueAsync(CancellationToken ct)
        {
            ...
            foreach (var eventType in _handlerTypes.Keys)
            {
                ...
                _logger.LogDebug("🔗 Activity Bound: {EventType}", eventType);
            }

            _logger.LogInformation("📊 Activity queue setup complete. {Count} event types", _handlerTypes.Count);
        }
        ...
        private async Task StartConsumingAsync(CancellationToken ct)
        {
            ...
            _logger.LogInformation("👂 Activity Service listening on: {Queue}", QueueName);
        }
        ...
        private async Task ProcessEventAsync(BasicDeliverEventArgs ea, CancellationToken ct)
        {
            ...
            try
            {
                ...
                _logger.LogDebug("📨 Activity Event: {EventType}", eventType);

                using var doc = JsonDocument.Parse(UnwrapPayload(json));
                
                if (!_handlerTypes.ContainsKey(eventType))
                {
                    _logger.LogWarning("⚠️ No activity handler: {EventType}", eventType);
                ...
                if (handler == null)
                {
                    _logger.LogWarning("⚠️ Activity handler not found in DI: {EventType}", eventType);
                    await _channel!.BasicAckAsync(ea.DeliveryTag, multiple: false, cancellationToken: ct);
                    return;
                }
                ...
                await _channel!.BasicAckAsync(ea.DeliveryTag, multiple: false, cancellationToken: ct);
                _logger.LogDebug("✅ Activity Processed: {EventType}", eventType);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "❌ Activity Failed: {EventType}", eventType);
        ...
        {
            _logger.LogInformation("🛑 Stopping Activity Service...");
